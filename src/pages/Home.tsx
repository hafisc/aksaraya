    ...
    <div className="min-h-screen">
        ...
        <Container>
          <motion.div variants={stagger} initial="initial" whileInView="animate" viewport={{ once: true }}>
            <div className="text-center mb-16">
              <Heading level={2} className="text-white mb-4" cultural>
                Koleksi Aksara Nusantara
              </Heading>
              <motion.p variants={fadeUp} className="text-white text-lg max-w-2xl mx-auto">
              ...
              {aksaraScripts.map((script) => (
                <motion.div key={script.id} variants={fadeUp}>
                  <Card hover className="text-center group">
                    <motion.div
                      className="text-6xl mb-4 text-white group-hover:text-white transition-colors duration-300"
                    >
                      {script.glyph}
                    </motion.div>
                    <h3 className="text-xl font-semibold text-white mb-2 font-cultural">
                      {script.name}
                    </h3>
                    <p className="text-white mb-3">{script.region}</p>
                    <p className="text-sm text-white">{script.description}</p>
                    <div className="mt-4">
                      <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${
                        script.status === 'active' ? 'bg-white text-[#592B18]' :
                        script.status === 'revitalized' ? 'bg-white text-[#592B18]' :
              ...
              {features.map((feature) => {
                ...
                return (
                  <motion.div key={feature.id} variants={fadeUp}>
                    <Card hover className="text-center group">
                      <div className="w-16 h-16 mx-auto bg-white rounded-full flex items-center justify-center mb-6 group-hover:bg-white transition-colors duration-300">
                        <Icon className="w-8 h-8 text-[#592B18]" />
                      </div>
                      <h3 className="text-xl font-semibold text-white mb-4 font-cultural">
                        {feature.title}
                      </h3>
                ...
                <motion.div key={testimonial.name} variants={fadeUp}>
                  <Card hover className="group">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-[#592B18] font-semibold">
      ...
      <Section background="brown">
        <Container>
          ...
          >
            <motion.div variants={fadeUp}>
              <Heading level={2} className="text-white mb-6" cultural>
